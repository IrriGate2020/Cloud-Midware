 ...
 * -------------------
 * Generated by     :: rafael
 * Generated at     :: Friday, August 14, 2026 at 6:19 PM Coordinated Universal Time
...
function extractPlanAssignment(scope) {
  ...
  return {
    ...
    custom_retention_days: getString(groupScope, [
      ...
      "dias_de_dados",
      "periodo_retencao",
      "periodo_reten\xE7\xE3o",
      "limite_retencao",
      "limite_reten\xE7\xE3o",
      "limite_retencao_dias",
      "limite_reten\xE7\xE3o_dias",
      "retention_limit",
      "data_retention_days"
    ]),
...
function getDeviceRetentionConfig2(retentionDays) {
  const days = Math.max(1, Math.floor(Number(retentionDays) || 1));
  const weeks = Math.min(26, Math.max(1, Math.ceil(days / 7)));
  return {
    chunk_period: "week",
    chunk_retention: weeks,
    warning: days > 182 ? "Retencao solicitada de " + days + " dias excede o maximo de 26 semanas da TagoIO. Aplicando 26 semanas." : void 0
  };
}
function getExistingDeviceRetentionConfig(retentionDays, _currentPeriod) {
  return getDeviceRetentionConfig2(retentionDays);
}
function formatRetentionConfig(retentionDays) {
  const config = getDeviceRetentionConfig2(retentionDays);
  return retentionDays + " dias (" + config.chunk_retention + " " + (config.chunk_retention === 1 ? "semana" : "semanas") + ")";
}
function buildDevicePlanTags(plan, organization) {
  const tags = [
    { key: "plan", value: plan.id },
    { key: "plano", value: plan.id },
    { key: "plan_device_limit", value: String(plan.deviceLimit) },
...
function userMatchesExplicitValue(user, value) {
  if (!value)
    return false;
...
async function applyRetentionToOrganizationDevices(resources, organization, plan, planTags, context) {
  const devices = await listOrganizationDevices(resources, organization);
  const desiredRetentionConfig = getDeviceRetentionConfig2(plan.retentionDays);
  ...
  for (const device of uniqueDevices.values()) {
    ...
    if (retentionConfig.warning)
      context?.log(retentionConfig.warning);
...
async function manageOrganizationPlan(context, scope) {
  ...
  const organizationRetentionConfig = getExistingDeviceRetentionConfig(plan.retentionDays, organizationDevice.chunk_period);
  const retentionConfig = organizationRetentionConfig;
  const retentionLabel = formatRetentionConfig(plan.retentionDays);
  context.log(`Organization retention apply: current_chunk_period=${organizationDevice.chunk_period || "none"} applied_chunk_period=${retentionConfig.chunk_period} applied_chunk_retention=${retentionConfig.chunk_retention}`);
  if (retentionConfig.warning)
    context.log(retentionConfig.warning);
  ...
  const organizationUsersMap = /* @__PURE__ */ new Map();
  users.filter((user) => userBelongsToOrganization(user, organization) || userMatchesExplicitValue(user, data.run_user)).forEach((user) => organizationUsersMap.set(user.id, user));
