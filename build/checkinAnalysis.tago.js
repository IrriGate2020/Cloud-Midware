 ...
 * -------------------
 * Generated by     :: rafael
 * Generated at     :: Monday, July 13, 2026 at 10:40 PM Coordinated Universal Time
...
async function checkinAnalysis(context, scope) {
  ...
  try {
            ...
            if (is_locked) {
              ...
              if (alert_metadata.send_to) {
                try {
                  ...
                  await sendRunNotification(resources, alert_metadata.send_to, `Alerta: Dispositivo Sem Comunica\xE7\xE3o`, `O dispositivo ${device_name} est\xE1 sem comunicar h\xE1 ${hours_offline.toFixed(1)} horas (limite: ${checkin_time_hours}h)`, context);
                } catch (error) {
    ...
    const central_devices = all_devices.filter((device) => {
      const type_tag = device.tags?.find((tag) => tag.key === "device_type");
      return type_tag && type_tag.value === "group";
    });
            ...
            if (is_locked) {
              ...
              if (alert_metadata.send_to) {
                try {
                  const device_info = await resources.devices.info(device_id);
                  const device_name = device_info.name || device_id;
                  await sendRunNotification(resources, alert_metadata.send_to, `Alerta: Central Sem Comunica\xE7\xE3o`, `A central ${device_name} est\xE1 sem comunicar h\xE1 ${hours_offline.toFixed(1)} horas (limite: ${checkin_time_hours}h)`, context);
                } catch (error) {
                  context.log(`Error sending notification: ${error}`);
                }
              }
              await resources.devices.sendDeviceData(central_device_id, {
                variable: "alert_triggered",
                value: "Comunica\xE7\xE3o da Central",
                metadata: {
                  alert_type: "checkin_central",
                  alert_variable: "checkin",
                  alert_variable_label: "Comunica\xE7\xE3o da Central",
                  device_id,
                  hours_offline,
                  checkin_time: checkin_time_hours,
                  timestamp: new Date().toISOString()
