...
async function createAlert(context, scope) {
  ...
  if (is_checkin_alert) {
    ...
    alert_description = `Alerta para o(a) ${alertData.alert_variable} do(a) ${device_name} quando o seu valor for ${condition_text} ${alertData.alert_value}${user_name ? ` ser\xE1 enviado para o usu\xE1rio ${user_name}` : ""}`;
  }
  ...
  if (alertData.alert_send_to) {
    try {
      const notification_message = is_checkin_alert ? `Novo alerta de checkin criado. Voc\xEA ser\xE1 notificado se o dispositivo ficar ${alertData.checkin_time} horas sem comunicar.` : `Novo alerta criado para monitorar o(a) ${alertData.alert_variable} no(a) ${device_name}`;
      await resources.run.notificationCreate(alertData.alert_send_to, {
        title: "Alerta Criado",
