...
async function createAlert(context, scope) {
  ...
  if (is_checkin_alert) {
    alert_description = `Alerta de checkin criado para monitorar comunica\xE7\xE3o do dispositivo. Notifica\xE7\xE3o ser\xE1 enviada se o dispositivo ficar ${alertData.checkin_time} horas sem comunicar${user_name ? ` e ser\xE1 enviado para o usu\xE1rio ${user_name}` : ""}`;
  } else {
  ...
  const organization_device_id = await resolveOrganizationDeviceId(resources, {
    explicitCandidates: [
      ...
      device_id !== group_device_id ? device_id : void 0
    ],
  ...
  if (alertData.alert_send_to) {
    try {
      const notification_message = is_checkin_alert ? `Novo alerta de checkin criado. Voc\xEA ser\xE1 notificado se o dispositivo ficar ${alertData.checkin_time} horas sem comunicar.` : `Novo alerta criado para monitorar ${variable_label} no(a) ${device_name}`;
      await resources.run.notificationCreate(alertData.alert_send_to, {
        title: "Alerta Criado",
