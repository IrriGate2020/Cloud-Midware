...
function getActiveErrorCodesFromValue(value: any, metadata: any): string[] {
    const textCodes = String(value || '').split(/[,|]/).map((item) => item.trim().match(/E[SA]\d{3}/)?.[0]).filter(Boolean) as string[];
...
function evaluateCondition(currentValue: any, condition: string, thresholdValue: any, currentMetadata: any = {}, variable: string = ""): boolean {
    ...
    for (const alert_data of central_alerts) {
        ...
        try {
            ...
            if (target_data.length > 0) {
                ...
                value_found = true;
            } else if (isErrorAlertVariable(alert_variable)) {
            ...
            const should_trigger = evaluateCondition(current_value, condition, threshold_value, current_metadata, alert_variable);
